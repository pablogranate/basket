...
- Facturación joins the sibling apps: the Auth DB enum `auth_app_access_app` gains `facturacion` (migration `drizzle/auth/0002`, run `npm run db:auth:migrate` on deploy). Nivel labels are now per app — in the Accesos matrix facturación offers Coordinador (`write`) and Admin (`admin`) only, `read` is refused server-side — and the landing links to `facturacion.basket-app.com`.
- Accesos page (`/access`, admins only): every identity in the Auth DB × every sibling app with a Nivel per cell; saving grants, changes or revokes the Acceso with `granted_by`/`granted_at`. Linked from Personal. Cutover seed `pnpm db:auth:seed-siblings` maps incidencias, ops hub and analytics users to Accesos and creates missing identities by email (dry-run, idempotent).
- Unified auth (ADR 0009): the Auth DB gains `auth_app_access` — one Acceso (Nivel `read|write|admin`) per identity and sibling app (`analytics|incidencias|generator|ops`), migration `drizzle/auth/0001`. The generator gate now admits any `generator` Acceso instead of the portal's full-access role; run `pnpm db:auth:seed-generator` after migrating so existing admins and editors keep it. `better-auth` is pinned to `1.6.15` (readers must match); new `db:auth:migrate` script; integration harness applies the Auth DB schema. Runbook: `docs/runbooks/unified-auth-acceso.md`.
