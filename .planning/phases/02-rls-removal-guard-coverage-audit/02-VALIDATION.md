...
| 02-01-04 | 01 | 1 | AUTHZ-01 | T-02-01 | non-admin denied reading app_settings.secret_value | unit | `npx vitest run src/lib/__tests__/settings-secret.test.ts` | ❌ W0 | ⬜ pending |
...
| 02-04-02 | 04 | 2 | AUTHZ-03 | T-02-11 | every action write path stamps + audits | source + type | `npm run typecheck && grep -rl writeAudit src/app/actions` | n/a | ⬜ pending |
| 02-04-03 | 04 | 2 | AUTHZ-03 | T-02-09 | collaborator-reports write produces non-NULL changed_by | unit | `npx vitest run src/lib/__tests__/audit.test.ts src/app/api/collaborator-reports` | ❌ W0 | ⬜ pending |
| 02-05-01 | 05 | 3 | AUTHZ-01 | T-02-12 | every route guarded + every loader ctx'd (structural) | unit (static) | `npx vitest run src/lib/api/__tests__/guard-coverage.test.ts` | ❌ W0 | ⬜ pending |
...
- AUTHZ-01: `with-auth.test.ts`, `guard-coverage.test.ts`, per-route `__tests__/auth.test.ts` (ai/*, intake), `loaders-ctx.test.ts`
...
- AUTHZ-03: `audit.test.ts` (changed_by never NULL, secret redaction, match_id rule)
- D-08: `settings-secret.test.ts`
...
|----------|-------------|------------|-------------------|
| Async RSC page render + role gating (`/people` etc.) | AUTHZ-01 | Vitest cannot test async Server Components (RESEARCH Pitfall 5) | Plan 05 Task 3: `npm run dev`, log in admin/non-admin, confirm pages render + gating + platform-access toggle |
| Live route 401/403/429 + intake machine-auth | AUTHZ-01 | End-to-end HTTP against running app | Plan 05 Task 3: curl guarded/guest/intake routes |
